...
export const updateArticle = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const article: any = await query("SELECT * FROM articles WHERE id = ?", [
      id,
    ]);
    if (article.length === 0)
      return res.status(404).json({ message: "Artigo não encontrado" });
    if (article[0].user_id !== userId)
      return res
        .status(403)
        .json({ message: "Não autorizado a editar este artigo" });
...
export const deleteArticle = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (article[0].user_id !== userId)
      return res
        .status(403)
        .json({ message: "Não autorizado a excluir este artigo" });
